...
#### 2. Blocking Unexplained Modifications to DeveloperTo effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Run this KQL Query:**
    ...
    | where FileName in~ (".bashrc", ".zshrc", ".bash_profile") or FolderPath contains ".ssh"
    ```
*   **Create Rule:** Click **Create detection rule**.
*   **Set ActionTo effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Run this KQL Query:**
    ...
    ```
*   **Create Rule:** Click **Create detection rule**.
*   **Set Action:** Choose to **Stop and Quarantine** the initiating file, or alert your SOC team with a High severity alert.

#### 3. Detecting Unexpected .toml orTo effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
#### 3. Detecting Unexpected .toml or .env Files with Shell Commands
Because Defender cannot easily read the plaintext *contents* of a `.toml` file natively at the moment of creation without heavy performance hits, you monitor the creation of these files in unusual contexts or pair this with Defender Application Control.

*   **Go to:** Hunting → Advanced Hunting.To effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Go to:** Hunting → Advanced Hunting.
*   **Run this KQL Query:** (This flags when these files are created unexpectedly by non-developer tools, or specifically by the compromised CLI).
    To effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Run this KQL Query:** (This flags when these files are created unexpectedly by non-developer tools, or specifically by the compromised CLI).
    ...
    | where FileName in~ ("config.toml", ".env")
    | where FolderPath contains ".codex"
    // Optional: Filter by specific suspicious initiating processes if known
    ```
*   **Create Rule:** Click **Create detection rule** andTo effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Run this KQL Query:** (This flags when these files are created unexpectedly by non-developer tools, or specifically by the compromised CLI).
    ...
    // Optional: Filter by specific suspicious initiating processes if known
    ```
*   **Create Rule:** Click **Create detection rule** and set it to alert so analysts can pull the file and extract the static hashes for Phase 1.
...
*   **Run this KQL Query:**
    ...
    DeviceNetworkEvents
    | where InitiatingProcessFileName in~ ("codex.exe", "codex")
    | where RemotePort !in (80, 443) // Assuming legitimate traffic is standard web traffic
    | where RemoteIPType == "Public"
    ```
*   **Create Rule:** Click **Create detection rule**.
*   **Set Action:** Choose **Isolate DeviceTo effectively block these threats in Microsoft Defender XDR, we need to make a slight adjustment to the approach. 
...
*   **Run this KQL Query:**
    ...
    ```
*   **Create Rule:** Click **Create detection rule**.
*   **Set Action:** Choose **Isolate Device** if you consider this a high-fidelity indicator of a supply chain compromise in progress.
